...
class HomepagePresenter extends BasePresenter
{
	...
	protected function createComponentUserDataGrid($name)
	{
		...
		$grid->onStatusChange[] = function ($id, $status) {
			// some logic and write to DB

			$this->flashMessage(sprintf('Status for ID: %s changed to %s.', $id, $status));
		...
		$grid->onSomeAction[] = function ($id) {
			// some logic and write to DB

			$this->flashMessage(sprintf('Some action with ID: %s', $id));
